---
title: Gemini, Cursor, Copilot, Kimi, Hermes, Pi, Oh My Pi, Antigravity
description: Use Gemini, Cursor, Copilot, Kimi, Hermes, Pi, Oh My Pi, and Antigravity in Vicoa by starting a new session directly from the web dashboard or mobile app.
---
...
| Pi | [Pi](https://github.com/earendil-works/pi) — `npm install -g @earendil-works/pi-coding-agent` |
| Oh My Pi | [Oh My Pi](https://github.com/can1357/oh-my-pi) — `brew install can1357/tap/omp` (it runs on Bun ≥ 1.3.14) |
| Antigravity | [Antigravity CLI](https://antigravity.google/docs/cli/install) — `curl -fsSL https://antigravity.google/cli/install.sh \| bash`, then run `agy` once and sign in with Google |
...
- **New agents don't appear?** They only show when both the CLI (≥ v1.6.1) and the app (≥ v1.6.8) are up to date — Pi and Oh My Pi need CLI ≥ v1.7.18. See [Version Compatibility](/docs/version-compatibility).
- **Oh My Pi refuses to start?** It runs on Bun, so the daemon checks for Bun ≥ 1.3.14 on your PATH before spawning it. Install Bun from [bun.sh](https://bun.sh), or use the Homebrew build, which bundles its own.
